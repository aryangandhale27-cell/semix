...
export const AdminOrdersTab: React.FC = () => {
  ...
  const [deliveryConfirmationOrderId, setDeliveryConfirmationOrderId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!deliveryConfirmationOrderId) return;

    setDeliveryCountdown(5);
    const timer = window.setInterval(() => {
      setDeliveryCountdown((remaining) => {
        if (remaining <= 1) {
          window.clearInterval(timer);
          return 0;
        }
  ...
  const confirmOrderDelivered = (orderId: string) => {
    if (deliveryConfirmationOrderId !== orderId || deliveryCountdown > 0) return;
    updateOrderStatus(orderId, 'delivered', 'Delivery confirmed by admin.', 'Admin');
    setDeliveryConfirmationOrderId(null);
  };
      ...
      <div className="space-y-3">
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => {
            ...
            const canMarkDelivered = order.status !== 'delivered' && order.status !== 'completed' && order.status !== 'cancelled';
                ...
                <div className="p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                    ...
                    >
                      ...
                      <span>{isExpanded ? 'Close Order' : 'Open Order'}</span>
                      {isExpanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </button>

                    {canMarkDelivered && (
                      deliveryConfirmationOrderId === order.id ? (
                        <div className="inline-flex items-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => confirmOrderDelivered(order.id)}
                            disabled={deliveryCountdown > 0}
                            className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-emerald-300 bg-emerald-600 px-3 py-2 text-xs font-bold text-white transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer"
                          >
                            <CheckCircle2 className="h-4 w-4" />
                            <span>{deliveryCountdown > 0 ? `Confirm in ${deliveryCountdown}s` : 'Confirm Delivered'}</span>
                          </button>
                          <button
                            type="button"
                            onClick={() => setDeliveryConfirmationOrderId(null)}
                            className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition-colors hover:bg-slate-50 cursor-pointer"
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => setDeliveryConfirmationOrderId(order.id)}
                          className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-emerald-300 bg-emerald-600 px-3 py-2 text-xs font-bold text-white transition-colors hover:bg-emerald-700 cursor-pointer"
                        >
                          <CheckCircle2 className="h-4 w-4" />
                  ...
                  <div className="bg-slate-50/70 border-t border-slate-200 p-4 sm:p-5 space-y-4">
                    <div>
                      <section className="rounded-xl border border-slate-200 bg-white p-4">
                        <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-800">Customer Delivery Details</h4>
                        <div className="grid grid-cols-1 gap-x-6 gap-y-2 text-xs sm:grid-cols-2">
                          ...
                          <a href={`tel:${order.customer.phone}`} className="inline-flex items-center gap-1.5 font-semibold text-[#561269] hover:underline">
                          ...
                          <p className="text-slate-500 sm:col-span-2">{order.customer.email}</p>
                        </div>
                      </section>
                    </div>
